const fs = require('fs');
const file = '/Users/shyamkumarpandey/.gemini/antigravity/scratch/qgis_reservation/frappe-bench/apps/qgis/frontend/src/pages/PropertyRecords.jsx';
...
// Extract start and end of RecordsPage
const lines = content.split('\n');
const start = lines.findIndex(l => l.startsWith('function RecordsPage('));
const end = lines.findIndex(l => l.startsWith('// ══════════════════════════════════════════════════════════════════════════\n// PROPERTY MASTER DETAIL PAGE'));

// Let's use string manipulation to replace RecordsPage
const before = lines.slice(0, start).join('\n');
const after = lines.slice(end - 1).join('\n'); // -1 to keep the empty line before the comment if there is one, actually let's just use the comment as boundary.

const newRecordsPage = `function RecordsPage({ userInfo, onView }) {
  ...
  const demandTotal = surveys.reduce((acc, s) => acc + (parseFloat(s.carpet_area) || 0) * 1000, 0) || 84000000;
...
const finalLines = [...lines.slice(0, start), newRecordsPage, ...lines.slice(lines.findIndex(l => l.startsWith('// ══════════════════════════════════════════════════════════════════════════\n// PROPERTY MASTER DETAIL PAGE')) === -1 ? end - 1 : lines.findIndex(l => l.startsWith('// ══════════════════════════════════════════════════════════════════════════\n// PROPERTY MASTER DETAIL PAGE')) - 1)]; // Actually, safer is to just find the start of the next section

let endIndexForAfter = lines.findIndex(l => l.includes('PROPERTY MASTER DETAIL PAGE'));
if (endIndexForAfter === -1) endIndexForAfter = end;
// Find the exact // line before PROPERTY MASTER DETAIL PAGE
while (endIndexForAfter > 0 && lines[endIndexForAfter - 1].startsWith('// ══')) {
    endIndexForAfter--;
}
...
const finalOutput = [
    ...
    newRecordsPage,
    '',
    ...lines.slice(endIndexForAfter)
].join('\n');

fs.writeFileSync(file, content.slice(0, content.indexOf('function RecordsPage(')) + newRecordsPage + '\n\n' + content.slice(content.indexOf('// ══════════════════════════════════════════════════════════════════════════', content.indexOf('function RecordsPage('))));

console.log('Successfully replaced RecordsPage');
